#!/usr/bin/env bash
# SessionStart hook for GrowingIO HarmonyOS SDK project.
#
...
#
# Aligned with superpowers/hooks/session-start design.
# Platform: macOS (bash 4+).

set -euo pipefail

# Resolve project root. Claude Code sets CLAUDE_PROJECT_DIR; fall back to
# walking up from this script for standalone testing.
...
if [ ! -f "$META_SKILL_PATH" ]; then
    # Fail loud via stderr so the problem is visible, but emit an empty-ish
    # JSON so the harness does not get malformed output.
...
You are operating in the GrowingIO HarmonyOS SDK project.
