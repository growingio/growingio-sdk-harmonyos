...
每个 step:
  kind    : click / longClick / back / ensure / wait
            - ensure: 确保界面上能看到 key（页面锚点），看不到就按返回键，最多 3 次。
              用它代替"盲按返回"，某一步失败时不会把导航栈退错层。
...
ENTRY_ANCHOR = 'go-dialogs'

STEPS = [
    step('click', 'entry', '在首页点击 [verify26]', 'verify26'),

    # ---------- 场景 1：主页 NavDestination ----------
    step('click', 'nav_home', '点击 [go-nav-home]', 'go-nav-home'),
    ...
    step('ensure', 'nav_home', '确认已回到 verify26 入口页', ENTRY_ANCHOR),
    step('wait', 'nav_home', '等待页面事件上报', ms=1000),

    # ---------- 场景 2：弹窗族 ----------
    step('click', 'dialog', '点击 [go-dialogs]', 'go-dialogs'),
    ...
    step('ensure', 'dialog', '确认回到弹窗页', 'open-sheet'),
    ...
    step('ensure', 'dialog', '确认回到弹窗页', 'long-press-me'),
    ...
    step('ensure', 'dialog', '确认已回到 verify26 入口页', ENTRY_ANCHOR),
    step('wait', 'dialog', '等待页面事件上报', ms=1000),

    # ---------- 场景 3：列表 index ----------
    step('click', 'list', '点击 [go-lists]', 'go-lists'),
    ...
    step('ensure', 'list', '确认已回到 verify26 入口页', ENTRY_ANCHOR),
